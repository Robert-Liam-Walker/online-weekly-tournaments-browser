...
export async function resultRoutes(app: FastifyInstance) {
  app.get("/events/:id/results", async (request, reply) => {
    ...
    const rows = await prisma.placement.findMany({
      ...
      orderBy: [{ roomIndex: "asc" }, { place: "asc" }],
    });
    const placements: PlacementDto[] = rows.map((r) => ({ userId: r.user.id, username: r.user.username, roomIndex: r.roomIndex, place: r.place, kos: r.kos, points: r.points }));
    return { event: { id: e.id, title: e.title, scheduledAt: e.scheduledAt.toISOString(), status: e.status }, placements };
  });

  app.get("/leaderboard", async () => {
    const grouped = await prisma.placement.groupBy({
      by: ["userId"],
      _sum: { points: true },
      _count: { _all: true },
      _min: { place: true },
    ...
    const rows: LeaderboardRow[] = grouped.map((g) => ({
      userId: g.userId,
      username: nameOf.get(g.userId) ?? "?",
      points: g._sum.points ?? 0,
      events: g._count._all,
      wins: winsOf.get(g.userId) ?? 0,
      bestPlace: g._min.place ?? 0,
    }));
    return { rows };
  });
  ...
  app.get("/users/:username/results", async (request, reply) => {
    ...
    return {
      ...
      results: rows.map((r) => ({ eventId: r.event.id, title: r.event.title, scheduledAt: r.event.scheduledAt.toISOString(), place: r.place, entrants: r.entrants, kos: r.kos, points: r.points })),
